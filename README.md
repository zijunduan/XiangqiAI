...
* `main.py`：项目入口，用于启动游戏或图形界面；
* `game.py`：棋局状态管理；
* `rules.py`：中国象棋规则与合法走法判断；
* `mcts.py`：蒙特卡洛树搜索相关代码；
* `model.py`：神经网络模型结构；
* `train.py`：训练流程代码；
* `train_smoke_eval.py`：快速训练/评估测试脚本；
* `font/`：字体资源；
* `image/`：棋盘与棋子图片资源；
* `docs/`：项目文档与截图。
...
本项目开发过程中使用了 ChatGPT 辅助进行代码结构建议、调试思路分析、README 文档组织和算法解释整理。项目的核心规则逻辑、棋盘状态建模、合法走法生成、Minimax / Alpha-Beta / MCTS 算法理解与整合均经过本人修改、测试和人工 Review。
